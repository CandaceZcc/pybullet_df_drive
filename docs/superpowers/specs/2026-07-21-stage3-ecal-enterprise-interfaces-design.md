...
- Protobuf 是阶段三必需依赖，本地模式也使用同一套消息编码。
- eCAL Python 运行时是正式接口必需依赖，但采用延迟导入，使未安装 eCAL 时仍可进入本地模式。
- 实施首先完成 eCAL 最小独立进程环回门禁；确认安装方式、Python API 和发布/订阅生命周期后，把成功版本及安装步骤固定到环境和文档。
...
`SimClock` 与 `PeriodicScheduler` 负责统一仿真时间、100 Hz 和 10 Hz 调度。调度使用累加期限，不按固定物理步数取模，避免 240 Hz 物理步长造成漂移。
...
前雷达、后雷达、RTK 和 IMU 分别调度、统计和捕获异常。一个话题失败时不发布半帧，也不阻止其他传感器发布。旧二维 LiDAR 摘要只可保留为内部日志或默认隐藏的开发者诊断，不进入企业消息或主 Dashboard。

## 7. Dashboard 与窗口布局

### 7.1 企业页面

保留现有 Qt 窗口、结构操作 FIFO、忙碌状态和障碍物表格。默认顶部页签为：

- `接口状态`：轮子、前雷达、后雷达、RTK、IMU。
- `障碍物`：逻辑 ID、模式、形状和位置。

只有配置显式启用时才创建 `开发者诊断` 页；默认界面不存在该页。
...
主页面移除位姿、车体速度、加速度、摩擦、接触、打滑、地形法向、旧 LiDAR 摘要、诊断曲线、相机参数和直接线速度/角速度控件。内部遥测、CSV、相机配置和诊断能力可继续保留。阶段四导航区域不提前创建。

### 7.2 接口状态

Dashboard 只读取不可变 `InterfaceStatusSnapshot`，Qt 线程不直接调用 eCAL。

- 全局显示传输模式和独立 eCAL 状态。
- 本地模式显示“本地测试模式”，同时明确显示“eCAL 未连接”。
- 轮子命令显示命令状态、有效接收频率和最新输入时间戳。
- 轮子状态显示实际轮速/转角数组、发布频率和最新仿真时间戳。
- 四个传感器分别显示话题状态、发布频率和最新仿真时间戳。
- 逐话题状态为：`活动`、`等待对端`、`超时`、`降级`、`未连接`、`错误`。

实际频率按最近 2 秒单调墙钟事件滚动计算，接收和发布分别统计。GUI 人工验收不以窗口打开时长作为频率判据；自动测试使用消息时间戳和计数。

### 7.3 暂停

暂停停止物理步进和仿真时钟，因此不生成新的轮子、点云、RTK 或 IMU 状态。Qt 事件、eCAL 对端发现和连接状态继续刷新。恢复前先检查命令墙钟年龄。

### 7.4 4:1 初始窗口组合

GUI 且 Dashboard 启用时，启动阶段读取主显示器 `availableGeometry`，排除任务栏和系统面板：
...
- X11/XWayland 验收环境使用现有 `xdotool` 按当前进程和 PyBullet 窗口标题定位 Main GUI；找不到窗口或无法应用几何时明确报告失败。
- Dashboard 禁用时 Main GUI 使用全部可用工作区。
- DIRECT 模式不初始化 Qt 屏幕或调用窗口工具。

布局和控件需在 1366x768、1920x1080、2560x1440 三种可用区域下验证无文本重叠；20% Dashboard 宽度不足时使用换行和纵向滚动，不破坏 4:1 比例。
...
3. 停止传感器调度和物理消息生成。
4. 刷盘并关闭接口日志。
5. 关闭 eCAL subscriber、publisher 和 participant。
6. 清理传感器临时资源并断开 PyBullet。
...
- 240 Hz 步进下长时间 100 Hz 与 10 Hz 累加调度，无固定步取模漂移。
...
- Dashboard 状态格式、2 秒频率窗口、企业字段边界和窗口布局计算。
...
- 验证 1366x768、1920x1080、2560x1440 下无文本重叠和不可达控件。
