...
from slope_sim.scene import TerrainBounds, create_slope_scene, probe_terrain, terrain_model_names
from slope_sim.simulation import _robot_base_height


TIME_STEP = 1.0 / 240.0
GROUND_ATTACHMENT_LIMIT_M = 0.06
MOVING_PATH_ERROR_LIMIT_M = 0.08
COLLISION_DISPLACEMENT_RATIO = 0.60
...
class PerformanceSummary:
    ...
    max_qt_event_seconds: float
    exceeded_soft_budget: bool
    exceeded_hard_event_limit: bool
    details: str
...
def robot_spawn_pose_matches(client_id: int, robot_id: int, scene, robot_model: str) -> bool:
    ...
    return max(abs(float(actual) - target) for actual, target in zip(position, expected)) <= ROBOT_SPAWN_POSITION_TOLERANCE_M
...
def batch_performance_passed(stats: PerformanceSummary) -> bool:
    """阶段二硬门禁：DIRECT 阻塞代理和 GUI 事件间隔均不超过 100 ms。"""
    return stats.max_step_seconds <= DIRECT_BLOCKING_SECONDS and not stats.exceeded_hard_event_limit
...
def _run_motion_check() -> VerificationCheck:
    ...
    try:
        ...
        passed = metrics.reversal_count >= 2 and metrics.max_path_error <= MOVING_PATH_ERROR_LIMIT_M
...
def _run_collision_gate_check() -> VerificationCheck:
    baseline = _forward_displacement(with_obstacle=False)
    blocked = _forward_displacement(with_obstacle=True)
    passed = baseline > 0.30 and blocked <= baseline * COLLISION_DISPLACEMENT_RATIO
    return VerificationCheck("collision_gate", passed, f"baseline={baseline:.3f} blocked={blocked:.3f}")
...
def _run_batch_performance_check() -> VerificationCheck:
    ...
    try:
        ...
        while not result.done:
            ...
            # DIRECT 验收没有真实 Qt，这里记录一次轻量事件泵占位，硬门禁仍由脚本输出判定。
            qt_event_durations.append(time.perf_counter() - event_start)
        if not result.succeeded:
        ...
        stats = performance_summary(
            ...
            qt_event_durations=qt_event_durations,
            soft_budget_seconds=SOFT_BUDGET_SECONDS,
...
def _forward_displacement(*, with_obstacle: bool) -> float:
    client_id = p.connect(p.DIRECT)
    try:
        scene = create_slope_scene(client_id, slope_deg=0.0, time_step=TIME_STEP, terrain_model="flat")
        if with_obstacle:
            manager = ObstacleManager(client_id, _settings_for_scene(scene), terrain_body_ids=scene.body_ids)
            from slope_sim.obstacles import ObstacleGeometry
            ...
            manager.restore(
                (
                    ObstacleSnapshot(
                        ...
                        "static",
                        "box",
                        (scene.spawn_position[0] + 0.75, scene.spawn_position[1], scene.spawn_position[2] + 0.35),
                        (0.0, 0.0, 0.0, 1.0),
                        geometry=ObstacleGeometry("box", (0.25, 0.35, 0.35)),
                    ),
                )
            )
        robot = create_robot(
