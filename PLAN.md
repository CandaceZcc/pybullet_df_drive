...
- [ ] 确认 Ubuntu 24.04 + Conda + Python 3.10 环境可用。
- [ ] 确认 `p.connect(p.DIRECT)` 可用。
- [ ] 确认 Ubuntu 本机 X11 下 `p.connect(p.GUI)` 可用。
- [ ] 确认 SSH / Remote SSH 下默认使用 `DIRECT`，不误用 GUI。
...
- [ ] 放置一个简单刚体。
- [ ] 记录刚体位置。
- [ ] 使用 Matplotlib 绘制位置变化。

### 阶段 2：差速底盘最小模型

- [ ] 参考 `thedeepestreality/pybullet_diffdrive` 创建或复制简化差速底盘 URDF。
- [ ] 加载底盘。
- [ ] 控制左轮和右轮速度。
- [ ] 获取底盘位置和 yaw 角。
- [ ] 记录底盘轨迹。
