...
class FakeLidarBackend:
    ...
    def __init__(self) -> None:
        self.parent_poses = {
            ...
            "lidar_link": Pose((0.0, 0.0, 0.0), IDENTITY_QUATERNION),
    ...
    def link_names(self) -> tuple[str, ...]:
        return ("base_link", "lidar_link", "lidar_front_mount", "lidar_rear_mount")
...
class ProductionStyleStage4Backend(FakeLidarBackend):
    """模拟生产后端在同一类声明公开观测入口与 ndarray 快路。"""

    def __init__(self) -> None:
        super().__init__()
        self.public_batch_calls = 0
        self.ndarray_batch_calls = 0

    def ray_test_batch(self, starts, ends, *, collision_mask: int):
        self.public_batch_calls += 1
        raise AssertionError("production Stage4 scan used the scalar public batch")
    ...
    def _ray_test_indexed_hits_ndarray(
        ...
        *,
        ...
        self.ndarray_batch_calls += 1
        assert starts.shape == ends.shape == (5_760, 3)
        ...
        assert collision_mask == LIDAR_VISIBLE_GROUP
        assert num_threads == 0
        return ()


class RecordingStage4Backend(ProductionStyleStage4Backend):
    """显式覆写公开入口，模拟需要保留完整射线观测的测试后端。"""

    def ray_test_batch(self, starts, ends, *, collision_mask: int):
        self.public_batch_calls += 1
        return FakeLidarBackend.ray_test_batch(
            self,
            starts,
            ends,
            collision_mask=collision_mask,
...
def test_default_lidar_scan_geometry_is_stable_and_immutable():
    ...
    assert config.max_range_m == 45.0
    assert config.ray_count == 2880
    with pytest.raises(AttributeError):
        config.vertical_lines = 8


def test_stage4_profile_exposes_only_the_fixed_progressive_firing_slot_count():
    """Stage4 profile 只描述冻结 progressive firing slots，不冒充规则角度网格。"""
    profile_type = getattr(lidar_pointcloud, "Stage4LidarProfile", None)
    assert profile_type is not None, "Stage4 lidar profile contract must exist"

    profile = profile_type.realtime()

    assert profile.firing_slot_count == profile.ray_count == 5_760
    assert not hasattr(profile_type, "dense")
    for old_grid_attribute in (
        "vertical_lines",
        "horizontal_samples",
        "horizontal_fov_deg",
        "vertical_fov_deg",
    ):
        assert not hasattr(profile, old_grid_attribute)


def test_stage4_lidar_scans_once_from_the_center_link_with_the_realtime_profile():
    """阶段四运行时只使用一个中心雷达，且一次射线批次固定为 5,760 条。"""
    scanner_factory = getattr(MultiLineLidar, "stage4", None)
    assert callable(scanner_factory), "Stage4 center lidar factory must exist"
    backend = FakeLidarBackend()

    scanner = scanner_factory(backend, lidar_pointcloud.Stage4LidarProfile.realtime())
    cloud = scanner.scan(100_000_000)

    assert cloud.frame_id == "lidar_link"
    assert cloud.lidar_id == 1
    assert backend.world_pose_calls == ["lidar_link"]
    assert len(backend.ray_batches) == 1
    assert len(backend.ray_batches[0][0]) == 5_760
    starts, ends, _mask = backend.ray_batches[0]
    assert math.dist((0.0, 0.0, 0.0), starts[0]) == pytest.approx(0.10)
    assert math.dist((0.0, 0.0, 0.0), ends[0]) == pytest.approx(45.0)


def test_stage4_production_style_backend_uses_same_class_ndarray_query() -> None:
    """生产类同层声明公开入口时仍必须避开 ndarray 到 tuple 的复制。"""
    backend = ProductionStyleStage4Backend()

    cloud = MultiLineLidar.stage4(
        backend,
        lidar_pointcloud.Stage4LidarProfile.realtime(),
    ).scan(100_000_000)

    assert cloud.points == ()
    assert backend.ndarray_batch_calls == 1
    assert backend.public_batch_calls == 0


def test_stage4_recording_subclass_override_keeps_public_batch_observable() -> None:
    """测试子类覆写公开入口后必须保留完整批次观测，不能被私有快路绕过。"""
    backend = RecordingStage4Backend()
    ...
    assert cloud.points == ()
    assert backend.public_batch_calls == 1
    assert backend.ndarray_batch_calls == 0
    assert len(backend.ray_batches) == 1
    assert len(backend.ray_batches[0][0]) == 5_760


def test_stage4_lidar_offsets_span_one_scan_period_for_the_full_realtime_profile():
    """5,760 条射线的 offset 必须按本 profile 分布，不能沿用旧 2,880 分母。"""
    backend = FakeLidarBackend()
    profile = lidar_pointcloud.Stage4LidarProfile.realtime()
    backend.hit_ray(0, local_point=(2.0, 0.0, 0.0), category="terrain")
    backend.hit_ray(
        profile.ray_count - 1,
        local_point=(2.0, 0.0, 0.0),
        category="terrain",
    )

    cloud = MultiLineLidar.stage4(backend, profile).scan(100_000_000)

    assert tuple(point.offset_time_ns for point in cloud.points) == (
        0,
        (profile.ray_count - 1) * LIDAR_SCAN_PERIOD_NS // profile.ray_count,
    )
    assert all(0 <= point.offset_time_ns < LIDAR_SCAN_PERIOD_NS for point in cloud.points)
...
def test_front_and_rear_use_frozen_mounts_and_transform_the_same_fixed_ray_table():
    ...
    for ray_index in (0, 89, 90, 179, 180, 2879):
        ...
        assert math.dist((0.0, 0.0, 0.0), front_ends[ray_index]) == pytest.approx(45.0)
...
def test_range_boundaries_are_inclusive_and_out_of_range_hits_are_removed():
    ...
    backend.hit_ray(2, local_point=(45.0, 0.0, 0.0), category="terrain")
    backend.hit_ray(3, local_point=(45.001, 0.0, 0.0), category="moving_obstacle")
...
def test_translated_rotated_exact_range_boundaries_survive_pybullet_round_trip():
    ...
    backend.hit_ray(
        maximum_index,
        local_point=tuple(value * 45.0 for value in rays[maximum_index]),
        category="terrain",
    )
...
def test_filtered_hits_keep_message_and_rotated_base_view_in_strict_same_order() -> None:
    ...
    backend.hit_ray(12, local_point=(46.0, 0.0, 0.0), category="static_obstacle")
