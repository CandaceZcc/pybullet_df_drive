...
def test_dashboard_marks_realtime_interface_inactive_when_lidar_tools_are_hidden(monkeypatch):
    """普通 runSim 关闭实时接口时不能把传感器误报成正在等待。"""
    monkeypatch.setenv("QT_QPA_PLATFORM", "offscreen")
    dashboard = TelemetryDashboard(show_lidar_tools=False)
    try:
        assert dashboard.ecal_status_label.text() == "未启用"
        assert "关闭实时接口" in dashboard.transport_detail_label.text()
        for row in dashboard.interface_rows.values():
            assert row.state_label.text() == "未启用"
