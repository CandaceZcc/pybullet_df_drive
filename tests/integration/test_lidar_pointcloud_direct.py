# 多线点云 DIRECT 集成测试：验证三地形、自身过滤、前后障碍标签和连续移动扫描。
...
from slope_sim.lidar_pointcloud import LidarConfig, MultiLineLidar, Stage4LidarProfile
...
STAGE4_LIDAR_SCAN_BUDGET_MS = 100.0
...
@pytest.mark.parametrize("terrain_model", ("flat", "slope", "golf_heightfield"))
def test_stage4_center_cloud_hits_terrain_without_hitting_robot(terrain_model):
    """中心 5,760 射线扫描必须只把真实可见表面转换为 lidar_link 点云。"""
    client_id = p.connect(p.DIRECT)
    try:
        scene, robot = _create_settled_world(client_id, terrain_model)
        backend = RecordingPyBulletSensorBackend(client_id, robot.robot_id)
        ...
        cloud = MultiLineLidar.stage4(
            backend,
            Stage4LidarProfile.realtime(),
        ).scan(100_000_000)

        (hits,) = backend.ray_history
        terrain_hits = sum(hit.body_id in scene.body_ids for hit in hits)
        assert all(hit.body_id != robot.robot_id for hit in hits)
        assert terrain_hits > 0
        assert cloud.frame_id == "lidar_link"
        assert cloud.lidar_id == 1
        assert cloud.point_num == len(cloud.points) == terrain_hits
        assert set(point.tag for point in cloud.points) == {1}
        assert all(
            math.isfinite(value)
            for point in cloud.points
            for value in (point.x, point.y, point.z)
...
@pytest.mark.parametrize("terrain_model", ("flat", "slope", "golf_heightfield"))
def test_stage4_center_cloud_is_valid_for_every_model_and_terrain(
    robot_model: str,
    terrain_model: str,
):
    """正式四车型三场地矩阵都必须保留中心雷达的可见地形点云。"""
    client_id = p.connect(p.DIRECT)
    try:
        scene, robot = _create_settled_world(client_id, terrain_model, robot_model)
        backend = RecordingPyBulletSensorBackend(client_id, robot.robot_id)
        backend.bind_scene(scene.body_ids, ())
        ...
        assert all(hit.body_id != robot.robot_id for hit in hits)
        assert terrain_hits > 0
        assert cloud.point_num == terrain_hits
    finally:
        p.disconnect(client_id)
    assert p.isConnected(client_id) == 0
...
def test_stage4_center_lidar_scan_fits_the_10hz_period():
    """中心 5,760 射线的完整真实扫描必须留在单个 10 Hz 周期内。"""
    client_id = p.connect(p.DIRECT)
    try:
        scene, robot = _create_settled_world(client_id, "flat")
        backend = PyBulletSensorBackend(client_id, robot.robot_id)
        ...
        for scan_index in range(5):
            started_at = time.perf_counter()
            scanner.scan(scan_index * 100_000_000)
            elapsed_ms.append((time.perf_counter() - started_at) * 1000.0)

        assert statistics.median(elapsed_ms) < STAGE4_LIDAR_SCAN_BUDGET_MS
    finally:
        p.disconnect(client_id)


def test_stage4_center_lidar_samples_at_10hz_during_a_240hz_headless_physics_window():
    """物理仍以 240 Hz 步进时，中心 LiDAR 必须每 24 步生成一个冻结样本。"""
    client_id = p.connect(p.DIRECT)
    try:
        scene, robot = _create_settled_world(client_id, "flat")
        backend = PyBulletSensorBackend(client_id, robot.robot_id)
        ...
        for step in range(240):
            ...
            p.stepSimulation(physicsClientId=client_id)
            if (step + 1) % 24 == 0:
                timestamp_ns = (step + 1) * 1_000_000_000 // 240
                timestamps.append(scanner.scan(timestamp_ns).timebase_ns)

        assert timestamps == [100_000_000 * index for index in range(1, 11)]
    finally:
        p.disconnect(client_id)
