# 真值传感器 DIRECT 集成测试：在四车型和三地形中对照 PyBullet 官方变换结果。
...
def test_default_mount_parent_links_exist_on_every_deliverable_model(robot_model):
    ...
    try:
        ...
        p.resetBasePositionAndOrientation(
            robot.robot_id,
            (-3.499, -0.009, 0.107),
            p.getQuaternionFromEuler((0.048, 0.005, 0.01)),
            physicsClientId=client_id,
        )
...
@pytest.mark.parametrize("robot_model", robot_model_names())
def test_stage4_three_point_rtk_and_center_lidar_mount_exist_on_every_model(robot_model):
    """四车型都必须能加载阶段四单 LiDAR 与三点 RTK 语义安装合同。"""
    client_id = p.connect(p.DIRECT)
    try:
        robot = create_robot(client_id, robot_model)
        backend = PyBulletSensorBackend(client_id, robot.robot_id)
        suite = Stage4TruthSensorSuite(backend, Stage4SensorMounts.default())

        rtk = suite.read_rtk(123_000_000)

        assert suite.mounts.lidar.parent_link == "lidar_link"
        assert "lidar_link" in backend.link_names()
        assert all(
            math.isfinite(value)
            for point in (rtk.left, rtk.center, rtk.right)
            for value in point
        )
        assert rtk.center == tuple(
            (rtk.left[index] + rtk.right[index]) / 2.0 for index in range(3)
        )
        assert -math.pi <= rtk.heading_rad < math.pi
    finally:
        p.disconnect(client_id)
    assert p.isConnected(client_id) == 0
