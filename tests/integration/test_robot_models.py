# 阶段一车型集成测试：锁定四种模型的结构、控语义和主动转向边界。
...
@pytest.mark.parametrize("model_name", robot_model_names())
def test_stage4_robot_models_expose_a_centered_lidar_link(model_name: str):
    """阶段四单中心 LiDAR 必须有稳定的语义 link，不能依赖旧前后挂点。"""
    root = ET.parse(get_robot_model(model_name).urdf_path).getroot()
    lidar_joint = root.find("./joint[@name='lidar_link_joint']")
    ...
    assert tuple(float(value) for value in lidar_joint.find("./origin").attrib["xyz"].split()) == pytest.approx(
        (0.0, 0.0, 0.105)
    )
