...
def test_sustained_backlog_ignores_constant_depth_when_writer_keeps_completing() -> None:
    """采样相位恒见一个新任务时，完成数前进可证明 writer 没有停滞。"""
    samples = (
        (0.0, 1, 10),
        (0.5, 1, 120),
        (1.1, 1, 240),
    ...
    assert verifier._has_sustained_backlog(samples) is False


def test_sustained_backlog_detects_monotonic_growth_despite_writer_progress() -> None:
    ...
    assert verifier._has_sustained_backlog(samples) is True


def test_sustained_backlog_detects_multi_item_plateau_after_idle() -> None:
    """从明确 idle 基线上升的多项平台不能伪装成稳定单项 in-flight。"""
    samples = (
        (0.0, 0, 0),
        (0.1, 8, 10),
        (0.6, 8, 50),
...
def test_sustained_backlog_does_not_backdate_observed_growth() -> None:
    """增长只能从首次观测时刻计时，不能回溯到上一个采样点。"""
    samples = (
        (0.0, 1, 10),
        (0.9, 2, 100),
        (1.0, 2, 110),
...
def test_sustained_backlog_detects_stalled_positive_inflight_item() -> None:
...
def test_sustained_backlog_times_growth_from_first_increase() -> None:
    """长期稳定单项后的短峰值不能被追溯成一秒增长。"""
    samples = (
        (0.7, 1, 271),
        (1.1, 1, 367),
        (1.5, 1, 463),
