...
def test_sustained_backlog_ignores_constant_depth_when_writer_keeps_completing():
    """采样相位恒见一个新任务时，完成数前进可证明不是同一项积压。"""
    samples = (
        (0.0, 1, 10),
        (0.5, 1, 120),
        (1.1, 1, 240),
    ...
    assert runtime_script._has_sustained_backlog(samples) is False


def test_sustained_backlog_detects_monotonic_growth_despite_writer_progress():
    """writer 有进展也不能掩盖连续一秒增长的队列深度。"""
    samples = (
        (0.0, 1, 10),
        (0.1, 2, 40),
        (1.2, 3, 240),
    ...
    assert runtime_script._has_sustained_backlog(samples) is True


def test_sustained_backlog_detects_multi_item_plateau_after_idle():
    """从 idle 跳到多项平台并持续一秒属于真实积压。"""
    samples = (
        (0.0, 0, 0),
        (0.1, 8, 10),
        (0.6, 8, 50),
...
def test_sustained_backlog_does_not_backdate_observed_growth():
    """首次观测增长不足一秒时不能借用前一个采样点的时间。"""
    samples = (
        (0.0, 1, 10),
        (0.9, 2, 100),
        (1.0, 2, 110),
...
def test_sustained_backlog_detects_stalled_positive_inflight_item():
    """正深度与完成数同时停滞一秒时必须报告真实阻塞。"""
    samples = (
        (0.0, 1, 10),
        (0.5, 1, 10),
        (1.1, 1, 10),
...
def test_sustained_backlog_times_growth_from_first_increase_not_stable_inflight():
    """长期稳定单项后的短促峰值不能被追溯成一秒增长。"""
    samples = (
        (0.7, 1, 271),
        (1.1, 1, 367),
        (1.5, 1, 463),
